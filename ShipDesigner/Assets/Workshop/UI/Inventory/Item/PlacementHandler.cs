...
namespace UI.Inventory.Item
{
	...
	public class PlacementHandler : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler
	{
		...
		public Workshop.Grid.Grid Grid;
		...
		void Start()
		{
			Grid = GameData.Instance.Grid.GetComponent<Workshop.Grid.Grid>();
		...
		{
			UpdateCurrentPosition();
			if(!ComponentSpawner.IsOccupied(CurrentPosition))
			{
				GameObject spawned = ComponentSpawner.SpawnObject(CurrentPosition);
				ComponentSpawner.AddToBlueprint(spawned);
			}
